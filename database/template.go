...
var (
	...
	ModelStruct *template.Template
	ObjApi      *template.Template
	QueryApi    *template.Template
	ManageApi   *template.Template
)

func init() {
	Header = template.Must(template.New("header").Parse(header))
	ModelStruct = template.Must(template.New("modelStruct").Parse(modelStruct))
	ObjApi = template.Must(template.New("objApi").Parse(objApi))
	QueryApi = template.Must(template.New("queryApi").Parse(queryApi))
	ManageApi = template.Must(template.New("managedApi").Parse(managedApi))
}
